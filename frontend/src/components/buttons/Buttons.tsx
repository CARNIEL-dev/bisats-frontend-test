...
export const PrimaryButton: React.FC<TButtons> = ({ css, text, loading, ...props }) => {
    return (
        <button type='submit' className={`h-[48px]  px-5 rounded-[6px] bg-[#F5BB00] text-[#0A0E12] text-[14px] leading-[24px] font-[600] text-center py-3 shadow-[0_0_0.8px_#000] ${props.disabled ? "bg-[lightGrey] text-grey" : ""} ${css}`}
            {...props}>
            {
                loading ? <DotLoader
                    color={"#0A0E12"}
...
export const WhiteTransparentButton: React.FC<TButtons> = ({ size, css, text, loading, ...props }) => {
    return (
        <button type='submit' className={`${css} ${size === "sm" ? "h-[32px] text-[12px] py-0.5 px-3" : "h-[48px] text-[14px] py-3"}  rounded-[6px] bg-transparent text-[#525C76] border-[1px] border-[#D6DAE1]  leading-[24px] font-[600] text-center py-3 `}
            {...props}>
            {
                loading ? <DotLoader
                    color={"#0A0E12"}
