...
import Warning from "../assets/icons/warning-toast.svg"
...
const BaseToast = ({ title, message, }: { title: string, message: string; }) => (
    <div className="-ml-[40px] " >
        <div className="text-[14px] leading-[24px]">
            ...
            <div className="text-[#606C82]">{message}</div></div>
    </div>
);

const Toast = {
    success: (message: string, title: string) =>
        toast.success(<BaseToast title={title} message={message} />, defaultOptions),

    warning: (message: string, title: string) =>
        toast.warning(<BaseToast title={title} message={message} />, defaultOptions),
