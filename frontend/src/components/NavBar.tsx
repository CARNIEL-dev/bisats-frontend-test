...
const NavBar = () => {
  ...
  return (
      <div className='bg-[#fff] lg:bg-[transparent] '>
          <header className="flex items-center justify-between px-5 lg:px-20 py-4 lg:py-4 relative z-10">
            <Link to="/" className='cursor-pointer'>
              ...
              <nav className="hidden lg:flex items-center gap-10  w-1/2">
                  {navLinks.map((link, index) => (
                      ...
                      <WhiteTransparentButton text={"Sign In"} loading={false} css="border-[1px] w-full  border-[#F3F4F6] bg-[#F6F7F8] -mr-4  text-[#181300]" onClick={() => navigate(APP_ROUTES.AUTH.LOGIN)} />

                      <PrimaryButton css='w-full' text={'Sign Up'} loading={false} onClick={() => navigate(APP_ROUTES.AUTH.SIGNUP)} />
              </nav>

              <div className='md:hidden'>
                  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" onClick={()=>setToggleMenu(!toggleMenu)}>
                      <path d="M3 7H21" stroke="#515B6E" stroke-width="1.5" stroke-linecap="round" />
                      <path d="M3 12H21" stroke="#515B6E" stroke-width="1.5" stroke-linecap="round" />
                      <path d="M3 17H21" stroke="#515B6E" stroke-width="1.5" stroke-linecap="round" />
                  </svg>
    ...
    ${toggleMenu ? 'translate-y-0 opacity-100' : '-translate-y-5 opacity-0 pointer-events-none'}`}
          >
              {navLinks.map((link, index) => (
                  <a
                      key={index}
