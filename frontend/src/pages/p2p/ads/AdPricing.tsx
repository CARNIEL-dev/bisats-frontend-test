...
const CreateAdPricing: React.FC<AdsProps> = ({
  formik,
  setStage,
  liveRate,
}) => {
  ...
  const handleNextStage = async () => {
    try {
      ...
      const requiredFields =
        pricingType.toLowerCase() === "static"
          ? ["pricingType", "price", "priceLowerLimit", "priceUpperLimit"]
          : [
              "pricingType",
              "priceLowerLimit",
              "priceUpperLimit",
  ...
  return (
    <>
      <div className="my-4">
        ...
        <p className="mt-1">
          ...
          <span className="text-[#515B6E] text-xs font-light">
            Static places a fixed price, while dynamic gives room for adjustment
            based fluctuations in market rates.
          </span>
        </p>
      </div>
      ...
      {pricingType === "Dynamic" ? (
        <>
          ...
          <p>
            <TriangleAlert color="#F59E0C" size={12} className="inline mr-1" />
            <span className="text-[#515B6E] text-xs font-light">
              This price is subject to change during fulfilment of this order
      ...
      <p className="text-[#515B6E] text-xs font-light mb-4">
        Market Price: 1 USDT ≈ {formatNumber(liveRate?.xNGN ?? 0)} xNGN
      </p>
