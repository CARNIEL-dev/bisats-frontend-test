...
const CreateAd = () => {
  ...
  const AdSchema = Yup.object().shape({
    ...
    price: Yup.number()
      ...
      .when(["asset", "$liveRate"], ([assetValue, liveRate], schema) => {
        ...
        return schema
          .min(
            minPrice ?? 1450,
            `Price must be greater than 90% of market rate`
          )
    ...
    maximumLimit: Yup.number()
      .when(
        ["minimumLimit", "amount", "amountToken"],
        ([minimumLimit], schema) =>
          schema.min(
    ...
    priceUpperLimit: Yup.number()
      .when(["priceLowerLimit", "price"], ([priceLowerLimit, price], schema) =>
        schema.min(
          Number(priceLowerLimit) + 1,
          "Upper price limit must be greater than minimum limit"
        )
      )
