...
const Level3Verification = () => {
  ...
  const SubmitFile2 = async () => {
    ...
    setIsLoading1(true);
    const response = (await Post_Proof_of_Wealth_KYC({
      userId: user?.user?.userId,
      file: file1,
    })) as APIResponseType;
    setIsLoading1(false);
  ...
  const SubmitFile3 = async () => {
    ...
    setIsLoading1(true);
    const response = (await Post_Proof_of_Profile_KYC({
      userId: user?.user?.userId,
      file: file1,
    })) as APIResponseType;
    setIsLoading1(false);
          ...
          {responsePending ? (
            <div className="flex flex-col justify-center items-center text-center space-y-2 my-4">
              <img
                ...
                alt="success"
              />
              <h1 className="text-[16px] text-[#515B6E]">
                Your documents have been uploaded successfully
              </h1>
              <p className="text-[14px] text-[#515B6E]">
            ...
            <div>
              <div className="bg-[#F9F9FB] p-2 mt-5 border border-[#F9F9FB] rounded-[8px] text-[12px] text-[#515B6E] w-full h-fit flex flex-col space-y-2 ">
                ...
                <p>
                  <span className="w-[4px] bg-[#C2C7D2] rounded-full  mr-1 h-[4px]"></span>
                  <span>
                    Max daily limit for withdrawal is unlimited NGN and 3m USD
                    in crypto
                  </span>
                </p>
              </div>
                ...
                <form onSubmit={Submit_Level3_kyc}>
                  <div>
                    <FileInput
                      ...
                      error={error1}
                      label={
                        "Upload a recent utility bill (Not later than 4months ago)"
                      }
                  ...
                  <div>
                    <FileInput
                      fileName={file3Name}
                      handleFileChange={handleFile3Change}
                      error={error1}
