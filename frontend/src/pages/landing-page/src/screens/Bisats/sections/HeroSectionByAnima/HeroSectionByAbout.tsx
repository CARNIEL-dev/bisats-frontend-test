...
export const HeroSectionAbout = ({
  ...
  return (
    <div
      className="h-[80vh] lg:max-h-[624px] w-full bg-[#0A0E12] dark:bg-background bg-cover bg-center bg-no-repeat relative overflow-hidden"
      style={{
    ...
    >
      <div className="relative z-10 h-full w-full flex flex-col lg:flex-row items-center justify-center gap-[20px] px-[20px] md:px-[40px] lg:px-[80px] my-[40px] lg:my-0">
        <div className="w-full max-w-[700px]">
          <h1
            className={`dark:text-foreground text-white font-medium text-[34px] lg:text-[42px] w-full ${titleAlignment} mb-[8px]`}
          >
            {title}
          </h1>
          {desc && (
            <p
              className={`font-normal text-[16px] lg:text-[18px] text-muted-foreground w-full ${descAlignment}`}
            >
              {desc}
            </p>
          )}
